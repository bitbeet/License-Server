...
- **在线公告**:后台发布静态公告(可附链接),支持全体发送 / 按机器码定向发送、即刻 / 定时发送;用户软件启动时随校验结果拿到,链接由软件端用默认浏览器打开
- **开放模式**:后台一键开关(即刻生效),开启期间新用户首次打开软件自动分发验证码并绑定机器码,无感进入;关闭后恢复手动输入
...
- **验证码页**:一键生成验证码,填备注(用户名)、选"永久有效"或指定到期时间;随时取消 / 恢复 / 删除 / 修改备注;顶部有统计数据,"最近校验"列即用户最后一次打开软件的时间;顶部有"开放模式"开关
- **公告页**:发布在线公告,可附链接(用户点击后软件用默认浏览器打开);发送对象可选全体或勾选多个已绑定机器码(带备注显示是谁);支持即刻发送与定时发送(北京时间);已发布的公告可"结束"(用户不再看到,记录保留)或删除
...
软件端对接的**开放接口文档**(校验 / 公告 / 健康检查,无需鉴权)见 **[docs/open-api.md](docs/open-api.md)**。
...
完整接口说明见 [docs/api.md](docs/api.md),软件端开放接口见 [docs/open-api.md](docs/open-api.md),通用软件接入见 [docs/integration.md](docs/integration.md),部署细节见 [docs/deploy.md](docs/deploy.md)。
